...
class reviewForm extends React.Component {
    ...
    console.log(diavisit)
    this.Auth.fetch('http://localhost:3000/diagnosisvisit/', {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
        },
        body: JSON.stringify(diavisit)
    }).then(response => {
        console.log('created diagnosisvisit');
        this.setState({
            diagnosisvisit: diavisit,
            submittedForm: true
        ...
        return (
            ...
            <br></br>
            {
               this.state.submittedForm === false ? '' : <Redirect to={{
                pathname: `/visit/${this.state.visit_id}`,
        //patient: this.state.foundPatient, 
        diagnosisvisit: this.state.diagnosisvisit
    }}
    />
}
</div>
